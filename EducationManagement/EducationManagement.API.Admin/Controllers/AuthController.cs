    ...
    public class AuthController : BaseController
    {
        ...
        public async Task<IActionResult> ForgotPassword([FromBody] ForgotPasswordRequest request)
        {
            ...
            try
            {
                ...
                // Gửi email OTP nếu SMTP đã được cấu hình
                ...
                {
                    email = email,
                    email_configured = _emailService.IsConfigured,
                    thoi_gian = DateTime.UtcNow
                });

                return Ok(new {
                    success = true,
                    message = _emailService.IsConfigured
                        ? "Mã OTP đã được gửi đến email của bạn. Vui lòng kiểm tra hộp thư."
                        : "Yêu cầu đặt lại mật khẩu đã được ghi nhận. Chức năng gửi email hiện chưa được cấu hình."
                });
            }
            catch (Exception ex)
            {
