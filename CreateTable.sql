...
CREATE TABLE dbo.academic_years (
    ...
    deleted_at         DATETIME NULL,
    deleted_by         VARCHAR(50) NULL,
    CONSTRAINT CHK_AcademicYear_StartBeforeEnd CHECK (start_year < end_year)
);
GO
...
CREATE TABLE dbo.school_years (
    ...
    deleted_at        DATETIME NULL,
    deleted_by        VARCHAR(50) NULL,
    CONSTRAINT CHK_SchoolYear_DateRange CHECK (start_date < end_date),
    CONSTRAINT CHK_SchoolYear_Semester1DateRange CHECK (
        semester1_start IS NULL OR semester1_end IS NULL OR semester1_start <= semester1_end
    ),
    CONSTRAINT CHK_SchoolYear_Semester2DateRange CHECK (
        semester2_start IS NULL OR semester2_end IS NULL OR semester2_start <= semester2_end
    ),
    CONSTRAINT CHK_SchoolYear_CurrentSemester CHECK (current_semester IS NULL OR current_semester IN (1, 2, 3))
);
GO
...
IF NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name = 'FK_Students_Advisor_Lecturers')
BEGIN
    ALTER TABLE dbo.students
    ADD CONSTRAINT FK_Students_Advisor_Lecturers
        FOREIGN KEY (advisor_id) REFERENCES dbo.lecturers(lecturer_id);
END
GO
...
CREATE TABLE dbo.classes (
    ...
    semester         INT NULL CHECK (semester IS NULL OR semester IN (1, 2, 3)),
...
CREATE TABLE dbo.enrollments (
    ...
    enrollment_status NVARCHAR(20) NOT NULL DEFAULT 'APPROVED',
    created_at      DATETIME NOT NULL DEFAULT(GETDATE()),
    created_by      VARCHAR(50) NULL,
    deleted_at      DATETIME NULL,
    deleted_by      VARCHAR(50) NULL,
    ...
    CONSTRAINT CHK_Enrollment_Status CHECK (enrollment_status IN ('PENDING', 'APPROVED', 'DROPPED', 'WITHDRAWN'))
);
GO

CREATE INDEX IX_Enrollments_Class_Student
ON dbo.enrollments (class_id, student_id)
INCLUDE (enrollment_id, enrollment_status, enrollment_date, deleted_at);
GO

-- ===========================================
-- 12. BẢNG ATTENDANCE_SESSIONS (Buổi điểm danh)
-- ===========================================
IF OBJECT_ID('dbo.TRG_AttendanceRecords_BlockWrite_WhenSessionLocked', 'TR') IS NOT NULL
    DROP TRIGGER dbo.TRG_AttendanceRecords_BlockWrite_WhenSessionLocked;
GO

IF OBJECT_ID('dbo.attendance_records', 'U') IS NOT NULL DROP TABLE dbo.attendance_records;
GO

IF OBJECT_ID('dbo.attendance_sessions', 'U') IS NOT NULL DROP TABLE dbo.attendance_sessions;
GO
...
CREATE TABLE dbo.attendance_sessions (
    ...
    lecturer_id        VARCHAR(50) NOT NULL FOREIGN KEY REFERENCES dbo.lecturers(lecturer_id),
    room_id            VARCHAR(50) NULL,
    timetable_session_id VARCHAR(50) NULL,
    week_no            INT NULL,
    ...
    CONSTRAINT CHK_AttendanceSession_TimeRange CHECK (start_time < end_time),
    CONSTRAINT CHK_AttendanceSession_WeekNo CHECK (week_no IS NULL OR week_no > 0),
    CONSTRAINT CHK_AttendanceSession_LockState CHECK (
        (is_locked = 0 AND locked_at IS NULL)
        OR
        (is_locked = 1 AND locked_at IS NOT NULL)
    ),
    CONSTRAINT UQ_AttendanceSession_Session_Class UNIQUE (session_id, class_id)
);
GO

CREATE UNIQUE INDEX UX_AttendanceSession_Class_Date_Time
ON dbo.attendance_sessions (class_id, session_date, start_time, end_time)
WHERE deleted_at IS NULL;
GO

CREATE INDEX IX_AttendanceSession_Class_Date
ON dbo.attendance_sessions (class_id, session_date)
INCLUDE (session_id, lecturer_id, start_time, end_time, is_locked, week_no, room_id);
GO

CREATE INDEX IX_AttendanceSession_Lecturer_Date
ON dbo.attendance_sessions (lecturer_id, session_date)
INCLUDE (session_id, class_id, start_time, end_time, is_locked);
GO

CREATE INDEX IX_AttendanceSession_LockStatus
ON dbo.attendance_sessions (is_locked, session_date, class_id)
INCLUDE (session_id, start_time, end_time);
GO

-- ===========================================
-- 12A. BẢNG ATTENDANCE_RECORDS (Kết quả điểm danh theo sinh viên)
-- ===========================================
CREATE TABLE dbo.attendance_records (
    ...
    CONSTRAINT FK_AttendanceRecords_Session FOREIGN KEY (session_id)
        REFERENCES dbo.attendance_sessions(session_id)
        ON DELETE CASCADE,
    CONSTRAINT FK_AttendanceRecords_Enrollment FOREIGN KEY (enrollment_id)
        REFERENCES dbo.enrollments(enrollment_id),
    CONSTRAINT FK_AttendanceRecords_Session_Class FOREIGN KEY (session_id, class_id)
        REFERENCES dbo.attendance_sessions(session_id, class_id),
    CONSTRAINT FK_AttendanceRecords_Enrollment_Class FOREIGN KEY (enrollment_id, class_id)
        REFERENCES dbo.enrollments(enrollment_id, class_id),
    CONSTRAINT UQ_AttendanceRecords_Session_Enrollment UNIQUE (session_id, enrollment_id),
    CONSTRAINT CHK_AttendanceRecords_Status CHECK (status IN (N'PRESENT', N'ABSENT', N'LATE', N'EXCUSED'))
);
GO

CREATE INDEX IX_AttendanceRecords_Session_Status
ON dbo.attendance_records (session_id, status)
INCLUDE (enrollment_id, checked_in_at, marked_at);
GO

CREATE INDEX IX_AttendanceRecords_Session_Enrollment
ON dbo.attendance_records (session_id, enrollment_id)
INCLUDE (status, note, checked_in_at, marked_at);
GO

CREATE INDEX IX_AttendanceRecords_Enrollment
ON dbo.attendance_records (enrollment_id)
INCLUDE (session_id, status, marked_at);
GO

CREATE TRIGGER dbo.TRG_AttendanceRecords_BlockWrite_WhenSessionLocked
...
BEGIN
    ...
    ;WITH affected_sessions AS (
        SELECT DISTINCT session_id FROM inserted
        UNION
        SELECT DISTINCT session_id FROM deleted
    )
    IF EXISTS (
        SELECT 1
        FROM affected_sessions s
        INNER JOIN dbo.attendance_sessions x
            ON x.session_id = s.session_id
        WHERE x.is_locked = 1
    )
    BEGIN
        ROLLBACK TRANSACTION;
        RAISERROR (N'Buổi điểm danh đã khóa, không thể thêm/sửa/xóa bản ghi.', 16, 1);
        RETURN;
    END
END
GO
...
CREATE TABLE dbo.grades (
    ...
    total_score   DECIMAL(4,2) NULL CHECK (total_score >= 0 AND total_score <= 10),
...
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'UQ_Grades_Enrollment_Active' AND object_id = OBJECT_ID('dbo.grades'))
BEGIN
    CREATE UNIQUE INDEX UQ_Grades_Enrollment_Active
...
CREATE TABLE dbo.notifications (
    ...
    deleted_at        DATETIME NULL,
    deleted_by        VARCHAR(50) NULL,
    CONSTRAINT CHK_Notifications_Type CHECK (type IN ('INFO', 'SUCCESS', 'WARNING', 'ERROR', 'SYSTEM'))
);
GO
...
CREATE TABLE dbo.refresh_tokens (
    ...
    token               VARCHAR(500) NULL, -- deprecated: giữ tạm để tương thích ngược, backend mới dùng token_hash
...
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'UX_Users_Username_Active' AND object_id = OBJECT_ID('dbo.users'))
    CREATE UNIQUE INDEX UX_Users_Username_Active ON dbo.users(username) WHERE deleted_at IS NULL;
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'UX_Users_Email_Active' AND object_id = OBJECT_ID('dbo.users'))
    CREATE UNIQUE INDEX UX_Users_Email_Active ON dbo.users(email) WHERE deleted_at IS NULL;
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'UX_Faculties_FacultyCode_Active' AND object_id = OBJECT_ID('dbo.faculties'))
    CREATE UNIQUE INDEX UX_Faculties_FacultyCode_Active ON dbo.faculties(faculty_code) WHERE deleted_at IS NULL;
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'UX_Faculties_FacultyName_Active' AND object_id = OBJECT_ID('dbo.faculties'))
    CREATE UNIQUE INDEX UX_Faculties_FacultyName_Active ON dbo.faculties(faculty_name) WHERE deleted_at IS NULL;
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'UX_Departments_DepartmentCode_Active' AND object_id = OBJECT_ID('dbo.departments'))
    CREATE UNIQUE INDEX UX_Departments_DepartmentCode_Active ON dbo.departments(department_code) WHERE deleted_at IS NULL;
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'UX_Majors_MajorCode_Active' AND object_id = OBJECT_ID('dbo.majors'))
    CREATE UNIQUE INDEX UX_Majors_MajorCode_Active ON dbo.majors(major_code) WHERE deleted_at IS NULL;
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'UX_Students_StudentCode_Active' AND object_id = OBJECT_ID('dbo.students'))
    CREATE UNIQUE INDEX UX_Students_StudentCode_Active ON dbo.students(student_code) WHERE deleted_at IS NULL;
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'UX_Lecturers_LecturerCode_Active' AND object_id = OBJECT_ID('dbo.lecturers'))
    CREATE UNIQUE INDEX UX_Lecturers_LecturerCode_Active ON dbo.lecturers(lecturer_code) WHERE deleted_at IS NULL;
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'UX_Subjects_SubjectCode_Active' AND object_id = OBJECT_ID('dbo.subjects'))
    CREATE UNIQUE INDEX UX_Subjects_SubjectCode_Active ON dbo.subjects(subject_code) WHERE deleted_at IS NULL;
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'UX_Classes_ClassCode_Active' AND object_id = OBJECT_ID('dbo.classes'))
    CREATE UNIQUE INDEX UX_Classes_ClassCode_Active ON dbo.classes(class_code) WHERE deleted_at IS NULL;
GO

-- Join indexes cho các cột join quan trọng
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_Users_RoleId' AND object_id = OBJECT_ID('dbo.users'))
    CREATE INDEX IX_Users_RoleId ON dbo.users(role_id);
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_Students_UserId' AND object_id = OBJECT_ID('dbo.students'))
    CREATE INDEX IX_Students_UserId ON dbo.students(user_id);
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_Students_MajorId' AND object_id = OBJECT_ID('dbo.students'))
    CREATE INDEX IX_Students_MajorId ON dbo.students(major_id);
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_Students_AdvisorId' AND object_id = OBJECT_ID('dbo.students'))
    CREATE INDEX IX_Students_AdvisorId ON dbo.students(advisor_id);
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_Lecturers_UserId' AND object_id = OBJECT_ID('dbo.lecturers'))
    CREATE INDEX IX_Lecturers_UserId ON dbo.lecturers(user_id);
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_Classes_SubjectId' AND object_id = OBJECT_ID('dbo.classes'))
    CREATE INDEX IX_Classes_SubjectId ON dbo.classes(subject_id);
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_Classes_LecturerId' AND object_id = OBJECT_ID('dbo.classes'))
    CREATE INDEX IX_Classes_LecturerId ON dbo.classes(lecturer_id);
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_Notifications_Recipient_Read_Created' AND object_id = OBJECT_ID('dbo.notifications'))
    CREATE INDEX IX_Notifications_Recipient_Read_Created ON dbo.notifications(recipient_id, is_read, created_at);
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_AuditLogs_User_Entity_Created' AND object_id = OBJECT_ID('dbo.audit_logs'))
    CREATE INDEX IX_AuditLogs_User_Entity_Created ON dbo.audit_logs(user_id, entity_type, created_at);
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_RefreshTokens_User_Expiry_Revoke' AND object_id = OBJECT_ID('dbo.refresh_tokens'))
    CREATE INDEX IX_RefreshTokens_User_Expiry_Revoke ON dbo.refresh_tokens(user_id, expires_at, revoked_at);
GO
...
-- Đồng bộ trạng thái cũ -> trạng thái chuẩn, sau đó loại bỏ cột cũ để tránh trùng nghĩa
IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('dbo.enrollments') AND name = 'status')
BEGIN
    UPDATE dbo.enrollments
    SET enrollment_status = CASE
        WHEN status IN ('PENDING', 'APPROVED', 'DROPPED', 'WITHDRAWN') THEN status
        ELSE enrollment_status
    END
    WHERE status IS NOT NULL;

    ALTER TABLE dbo.enrollments DROP COLUMN status;
    PRINT '✓ Dropped deprecated column: enrollments.status';
END
GO
...
BEGIN
    ...
    BEGIN
        ...
        ;WITH AnchorPairs AS (
            SELECT DISTINCT
                i.subject_id AS root_subject_id,
                i.prerequisite_subject_id
            FROM inserted i
            WHERE i.subject_id IS NOT NULL AND i.prerequisite_subject_id IS NOT NULL
        ),
        PrereqChain AS (
            SELECT
                ap.root_subject_id,
                ...
                1 AS depth
            FROM AnchorPairs ap
            INNER JOIN subject_prerequisites sp
                ON sp.subject_id = ap.prerequisite_subject_id
            WHERE sp.is_active = 1 AND sp.deleted_at IS NULL
            ...
            FROM PrereqChain pc
            INNER JOIN subject_prerequisites sp
                ON sp.subject_id = pc.prerequisite_subject_id
        ...
        IF EXISTS (
            SELECT 1
            FROM PrereqChain pc
            WHERE pc.prerequisite_subject_id = pc.root_subject_id
        )
        BEGIN
            ROLLBACK TRANSACTION;
...
IF NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name = 'FK_AttendanceSessions_Room')
BEGIN
    ALTER TABLE dbo.attendance_sessions
    ADD CONSTRAINT FK_AttendanceSessions_Room
...
IF NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name = 'FK_AttendanceSessions_TimetableSession')
BEGIN
    ALTER TABLE dbo.attendance_sessions
    ADD CONSTRAINT FK_AttendanceSessions_TimetableSession
        FOREIGN KEY (timetable_session_id) REFERENCES dbo.timetable_sessions(session_id);
END
GO
