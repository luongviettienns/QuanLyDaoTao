...
namespace EducationManagement.DAL.Repositories
{
    public class GradeRepository
    {
        ...
        private const string GradeSelect = @"
            SELECT
                g.grade_id, g.enrollment_id, g.midterm_score, g.final_score, g.attendance_score,
                g.assignment_score, g.total_score, g.letter_grade, g.created_at, g.created_by,
                g.updated_at, g.updated_by,
                e.student_id, e.class_id,
                s.student_code, s.full_name AS student_name,
                c.class_code, c.class_name, c.school_year_id, c.semester,
        ...
        public async Task<List<Grade>> GetAllAsync()
        {
            ...
            var dt = await DatabaseHelper.ExecuteRawQueryAsync(
                _connectionString,
                GradeSelect + @"
                ORDER BY g.created_at DESC, s.student_code, c.class_code");
        ...
        public async Task<List<Grade>> GetByStudentIdAsync(string studentId)
        {
            ...
            var dt = await DatabaseHelper.ExecuteRawQueryAsync(
                ...
                ORDER BY g.created_at DESC, c.class_code",
                new SqlParameter("@StudentId", studentId));
        ...
        public async Task<List<Grade>> GetByClassIdAsync(string classId)
        {
            ...
            var dt = await DatabaseHelper.ExecuteRawQueryAsync(
                ...
                ORDER BY s.student_code, g.created_at DESC",
                new SqlParameter("@ClassId", classId));
