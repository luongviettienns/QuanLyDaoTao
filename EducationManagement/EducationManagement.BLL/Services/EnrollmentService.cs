...
namespace EducationManagement.BLL.Services
{
    public class EnrollmentService
    {
        ...
        public async Task<List<EnrollmentDetailDto>> GetEnrollmentsByStudentAsync(string studentId)
        {
            if (string.IsNullOrWhiteSpace(studentId))
                throw new ArgumentException("Student ID không được để trống");

            return await _enrollmentRepository.GetEnrollmentsByStudentDetailedAsync(studentId);
        }
